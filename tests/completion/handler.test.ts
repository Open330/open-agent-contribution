...
vi.mock("../../src/completion/github-pr.js", () => ({
...
vi.mock("../../src/completion/issue-linker.js", () => ({
  linkIssueToePR: vi.fn().mockResolvedValue(undefined),
}));

import { validateDiff } from "../../src/completion/diff-validator.js";
import { createPR, pushBranch } from "../../src/completion/github-pr.js";
import { CompletionHandler, type CompletionHandlerParams } from "../../src/completion/handler.js";
...
/**
 * Creates a mock Octokit instance with `pulls.list` stubbed.
 * Default: returns empty pull list (no duplicates).
 */
function makeOctokit(pullsData: unknown[] = []): Octokit {
  return {
    pulls: {
...
describe("CompletionHandler", () => {
  ...
  // ── OAC Concurrency: Layer 3 — duplicate PR guard ──────────

  it("skips PR creation when a duplicate OAC PR already exists", async () => {
    const oacPulls = [
      {
        number: 42,
        title: "[OAC] Fix issue #5",
        body: "Fixes #5\n\n---\n*Auto-generated by OAC.*",
        state: "open",
      },
    ];
    const handler = new CompletionHandler({
      octokit: makeOctokit(oacPulls),
      eventBus: createEventBus(),
    });
    const params = makeParams({
      task: makeTask({
  ...
  it("proceeds normally when no duplicate OAC PR exists", async () => {
    const handler = new CompletionHandler({
      octokit: makeOctokit([]),
      eventBus: createEventBus(),
    });
  ...
  it("skips guard check when task has no linkedIssue", async () => {
    ...
    const params = makeParams({
      task: makeTask({ linkedIssue: undefined }),
    });
  ...
  it("does not block PR creation when the guard throws", async () => {
    const octokit = {
      pulls: {
        list: vi.fn().mockRejectedValue(new Error("API rate limited")),
      },
    } as unknown as Octokit;
    const handler = new CompletionHandler({
      octokit,
